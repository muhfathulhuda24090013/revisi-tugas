...
export const FaqItem = ({ question, answer, aosDelay }: FaqItemProps) => {
  const [isOpen, setIsOpen] = useState(false);

  return (
    <div data-aos="zoom-in-down" data-aos-delay={aosDelay}>
      <div className="mb-8 w-full rounded-lg p-4 shadow-[0px_0px_7px_0px_rgba(153,5,81,1)] overflow-hidden relative before:absolute before:w-[0.4rem] before:h-full before:bg-invofest before:right-0 before:top-0 sm:p-8 lg:px-6 xl:px-8">
        ...
        >
          <div className="mr-5 flex h-10 w-full max-w-[40px] items-center justify-center rounded-lg text-slate-600 bg-gray-600/5">
            <svg className={`fill-slate-600 stroke-slate-600 duration-200 ease-in-out ${isOpen ? 'rotate-180' : ''}`} width="17" height="10" viewBox="0 0 17 10" xmlns="http://www.w3.org/2000/svg">
          ...
          <div className="w-full">
            <h4 className="mt-1 text-lg font-semibold text-slate-600">{question}</h4>
          </div>
        </button>
        <div className={`pl-[62px] duration-200 ease-in-out ${isOpen ? 'block' : 'hidden'}`}>
          <p className="py-3 text-base leading-relaxed text-slate-600">{answer}</p>
        </div>
      </div>
    </div>
  );
